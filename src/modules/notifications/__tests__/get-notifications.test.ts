...
describe("GET /api/notification — pagination validation", () => {
  const app = createApp();
  ...
  it("returns 400 when pageSize exceeds the max of 100", async () => {
    ...
    const res = await request(app)
      .get("/api/notification?pageSize=101")
      .set("Cookie", session.cookie)
      .set("Accept", "application/json");
  ...
  it("defaults pageSize to 10 when omitted (no longer the old 50)", async () => {
    const session = await createAdminSession();
    ...
    expect(res.status).toBe(200);
    expect(res.body.data[0].pageSize).toBe(10);
  });
});
